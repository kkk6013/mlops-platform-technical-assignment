...
def approve_version(model_id: str, version_id: str):
    ...
    """Approve a version, allowing it to be promoted to live stages later."""
...
def promote_version(model_id: str, version_id: str, payload: PromoteRequest):
    ...
    """Promote a version to a stage. STAGING/PRODUCTION require approval first."""
    ...
    if payload.stage in _STAGES_REQUIRING_APPROVAL and not version.approved:
        ...
        raise HTTPException(
            ...
            detail=f"Version {version_id} must be approved before promotion to {payload.stage.value}.",
...
def create_deployment(payload: DeploymentRequest):
    """Create a deployment. Enforces: only approved versions can deploy.
    Supports idempotency keys to handle duplicate requests safely."""
    if payload.idempotency_key and payload.idempotency_key in _idempotency_keys:
        ...
        return _deployments[existing_id]

    version = _find_version(payload.model_id, payload.version_id)
    if not version.approved:
        raise HTTPException(
            status_code=409,
            detail=f"Version {payload.version_id} must be approved before deployment.",
        )
